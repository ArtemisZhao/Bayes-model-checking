...
posterior_checking<-function(beta, se, r_vec = c(1e-5, 6e-3, 0.024), L= 1000, test="skew", print_test_dist=FALSE) {
...
run_analysis<-function(x){
	...
	return(posterior_checking(beta,se,test="Q"))
}
...
pvec = apply(d, 1, run_analysis)

outfile = paste0("output/batch_multigrp.bb_",round(bb,2),".rnse_",rnse,".bt_",bt,".pdf")
